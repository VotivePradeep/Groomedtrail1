...
class paypal_lib {
	...
	function __construct()
	{
		...
		//13-02-2018 $sanbox = $this->CI->config->item('sandbox');
         $paypal_credential = paypal_credential();
         if($paypal_credential[0]['config_type'] == 'Live'){
          	$sanbox = FALSE;
          }else{
          	$sanbox =TRUE;
          }
		$this->paypal_url = ($sanbox == TRUE)?'https://www.sandbox.paypal.com/webapps/hermes':'https://www.paypal.com/webapps/hermes';
		...
		$businessEmail = $paypal_credential[0]['paypal_email'];
		$this->add_field('business',$businessEmail);
		$this->add_field('rm','2');			  // Return method = POST
		$this->add_field('cmd','_xclick');

		$this->add_field('currency_code', $paypal_credential[0]['paypal_default_currency']);
