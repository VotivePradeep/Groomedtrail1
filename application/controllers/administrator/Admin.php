...
function kmlmanagement() 
	 {
		...
		$data['basesegment']= $this->uri->segment(2);
		$data['segment'] = $this->uri->segment(3);
		$data['trailList'] = $this->model->get_data('*', 'tbl_trail_type_master', array('trail_status' =>1));
       ...
       if($data['segment'] == 'editpoi') {
	    ...
	    //$data['poiDetail'] = $this->model->join_where('tbl_trail_master.*, tbl_kml_data.*', 'tbl_trail_master','tbl_kml_data','trail_type_id','trail_fk_id',$where, 'kml_data_id', 'DESC');
	  	...
	  	if($this->form_validation->run('editpoi') == FALSE) {
			...
			$addpoi['lat_lang']=$this->input->post('lng').','.$this->input->post('lat').',0';
...
public function changeTrailStatusChange(){
	...
	$array2 = array(
	     $field2=>$value2,$wherevalue4=>$wherefield4	 
	);
	$where2 = array(
	...
	echo $value;
        ...
        $this->model->update('tbl_kml_data_trail',array('trail_dscrptn'=>$userUpdateTrail->trail_description),array('klm_trail_name'=>$wherevalue2));
		...
		$message = str_replace("{trail_update_details}",ucfirst($userUpdateTrail->trail_description),$message);
		$this->sendingMail($username->email,$subject, $message);
...
public function changeTrailStatus(){
	...
	function traillist() 
	 {
			...
			//$data_get=array("select"=>"*","table"=>"tbl_trail_report","table2"=>"tbl_kml_data_trail", "equal"=>"tbl_trail_report.trail_name = tbl_kml_data_trail.klm_trail_name", "RIGHT_LEFT"=>'RIGHT',"table3"=>"","equal3"=>"","table4"=>"","equal4"=>"","search_record"=>"tbl_kml_data_trail.klm_trail_name","group_by_desc"=>"tbl_kml_data_trail.klm_trail_name", "search_order"=>"tbl_kml_data_trail.id", "search_default_order"=>"tbl_kml_data_trail.id");
			 $data_get=array("select"=>"*","table"=>"tbl_trail_report","table2"=>"tbl_kml_data_trail", "equal"=>"tbl_trail_report.trail_name = tbl_kml_data_trail.klm_trail_name","RIGHT_LEFT"=>'RIGHT', "table3"=>"","equal3"=>"","table4"=>"","equal4"=>"","search_record"=>"tbl_kml_data_trail.klm_trail_name", "group_by_desc"=>"tbl_kml_data_trail.klm_trail_name","search_order"=>"tbl_kml_data_trail.id", "search_default_order"=>"tbl_kml_data_trail.id");

            $totalRec = count($this->model->getRowsJoinMulti($data_get));
			$config['target']      = '#postList';
			$config['base_url']    = base_url().'administrator/admin/ajaxtraillistPaginationData';
			$config['total_rows']  = $totalRec;
            $config['per_page']    = $this->perPage;
            $config['link_func']   = 'searchFilter';
            $this->ajax_pagination->initialize($config);
		
			$data['kmlList'] = $this->model->getRowsJoinMulti($data_get, array('limit'=>$this->perPage));
	...
	function ajaxtraillistPaginationData()
    {
        ...
        if(!empty($per_page)){
			 ...
			 $limit_data = $this->perPage;
		}
		if(!empty($state)){
            $conditions['search']['where_state_value'] = $state;
        }
        if(!empty($keywords)){
            $conditions['search']['keywords'] = $keywords;
        }
        //total rows count
        $data_get=array("select"=>"*","table"=>"tbl_trail_report","table2"=>"tbl_kml_data_trail", "equal"=>"tbl_trail_report.trail_name = tbl_kml_data_trail.klm_trail_name","RIGHT_LEFT"=>'RIGHT', "table3"=>"","equal3"=>"","table4"=>"","equal4"=>"","search_record"=>"tbl_kml_data_trail.klm_trail_name", "search_order"=>"tbl_kml_data_trail.id","group_by_desc"=>"tbl_kml_data_trail.klm_trail_name", "search_default_order"=>"tbl_kml_data_trail.id","where_state"=>"tbl_kml_data_trail.region_name");
        
        $totalRec = count($this->model->getRowsJoinMulti($data_get, $conditions));
        $config['target']      = '#postList';
        $config['base_url']    = base_url().'administrator/admin/ajaxtraillistPaginationData';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $limit_data;
        $config['link_func']   = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        //set start and limit
        $conditions['start'] = $offset;
        $conditions['limit'] = $limit_data;
        $data['kmlList'] = $this->model->getRowsJoinMulti($data_get, $conditions);
        //load the view
        $this->load->view('administrator/traillist/ajax_traillist_pagination', $data, false);
    }
    function pandingtraillist() 
	 {
		 	$info=$this->session->all_userdata();
			$data['userinfo']=$info;
			$data['u_id'] = $this->data['user_id'];
			...
			$query = $this->db->query('SELECT * FROM tbl_kml_data_trail group by klm_trail_name ORDER BY `id` ASC');
			$data['stateList'] = $query->result();
			$data_get=array("select"=>"*","table"=>"tbl_trail_report","table2"=>"tbl_kml_data_trail_view", "equal"=>"tbl_trail_report.trail_name = tbl_kml_data_trail_view.klm_trail_name","table3"=>"","equal3"=>"","table4"=>"","equal4"=>"","search_record"=>"tbl_kml_data_trail_view.klm_trail_name","group_by_desc"=>"", "search_order"=>"`tbl_trail_report`.`trail_report_id`", "search_default_order"=>"`tbl_trail_report`.`trail_report_id`", "where_trail_status"=>"`tbl_trail_report`.`status`");

            $totalRec = count($this->model->getRowsJoinMulti($data_get));
			$config['target']      = '#postList';
			$config['base_url']    = base_url().'administrator/admin/ajaxpandingtraillistPaginationData';
			$config['total_rows']  = $totalRec;
            $config['per_page']    = $this->perPage;
            $config['link_func']   = 'searchFilter';
            $this->ajax_pagination->initialize($config);
		
			$data['ViewPendingUpdates'] = $this->model->getRowsJoinMulti($data_get, array('limit'=>$this->perPage));


			/*
        	$query1 = $this->db->query('SELECT tbl_kml_data_trail_view.*, `tbl_trail_report`.* FROM `tbl_trail_report` JOIN `tbl_kml_data_trail_view` ON `tbl_trail_report`.`trail_name`=`tbl_kml_data_trail_view`.`klm_trail_name` WHERE `tbl_trail_report`.`status`=0 ORDER BY `tbl_trail_report`.`trail_report_id` DESC');

			$data['ViewPendingUpdates'] = $query1->result();*/

			$this->load->view('administrator/include/inner_header',$data);
			$this->load->view('administrator/traillist/pandingtraillist',$data);
			$this->load->view('administrator/include/inner_footer');

	 }

	function ajaxpandingtraillistPaginationData()
    {
        $page = $this->input->post('page');
        if(!$page){
            $offset = 0;
        ...
        $data['ViewPendingUpdates'] = $this->model->getRowsJoinMulti($data_get, array('limit'=>$this->perPage));
        //load the view
        $this->load->view('administrator/traillist/ajax_pandingtraillist_pagination', $data, false);
    }
	  function trails(){
		$info=$this->session->all_userdata();
		$data['userinfo']=$info;
	...
	public function trail_acitities()
	{
		...
		if($inbox_action == 'CloseALLTrails'){
	        ...
	        $sql = 'UPDATE `tbl_kml_data_trail` SET '.$set_value.' WHERE region_name="'.$region_name.'"';
	...
	function trailreport() 
	 ...
	 function pending_submissions_trail_report() 
	 {
		 	$info=$this->session->all_userdata();
			$data['userinfo']=$info;
			$data['u_id'] = $this->data['user_id'];
			...
			if($data['segment'] == 'trailreportedit'){
				...
				$this->load->view('administrator/include/inner_footer');
				    ...
				    if($update){
				       $this->session->set_flashdata('success', 'The Trail report is Successfully update');
				       redirect(base_url().'administrator/pending_submissions_trailreport');
			        }				   
				}
			}else{
			    $this->load->view('administrator/include/inner_header',$data);
		        $this->load->view('administrator/traillist/approve_rejected_trailreport',$data);
		        $this->load->view('administrator/include/inner_footer');
	 ...
	 public function trail_report_acitities()
	{
		...
		if($inbox_action == 'OpenALLTrails'){
	        ...
	        $sql = 'UPDATE `trail_report_user_update` SET '.$set_value.' WHERE CountyID IN ('.$inbox_ids.')';
            $marks_as_read = $this->db->query($sql);
            $query1 = $this->db->query("SELECT tbl1.*, tbl2.trail_report_created_date FROM trail_report_user_update AS tbl1 LEFT OUTER JOIN trail_report_user_update AS tbl2 ON tbl2.CountyID = tbl1.CountyID AND tbl2.trail_report_created_date < tbl1.trail_report_created_date WHERE tbl1.CountyID IN (".$inbox_ids.") AND tbl1.trail_report_status= 1 GROUP BY tbl1.CountyID");

			$getUpdatedTrailDetails = $query1->result();
			
			foreach ($getUpdatedTrailDetails as $r) {
				
			$username = $this->model->get_row('tbl_user_master', array('user_id'=>$r->userID));
			$state_name = $r->state_name;
			$countyName = $r->CountyID;
			$trail_report_conditions =$r->trail_report_conditions;
			$u_name = $username->fname.' '.$username->lname;
			$username = $this->model->get_row('tbl_user_master', array('user_id'=>$userUpdateTrail->userID));
			$update = $this->model->update('county_trail_report',array('submitted_by' => $u_name, 'trail_conditions'=>$trail_report_conditions),array('region_name' =>$state_name, 'county_name' =>$countyName));
			if($update){

			$row1 =  $this->model->get_row('email_templates',array('title'=>'TrailAndTrailReportUpdate'));
			$subject = $row1->subject;
			$message = $row1->content;
			$message = str_replace("{username}",ucfirst($u_name),$message);
			$message = str_replace("{trail_type}",'Trail Report',$message);
			$message = str_replace("{user_email_address}",$username->email,$message);
			$message = str_replace("{trail_name}",ucfirst($countyName),$message);
			$message = str_replace("{trail_update_details}",ucfirst($trail_report_conditions),$message);
	 ...
	 public function trail_report_acitities1()
	{
		$inbox_ids = $this->input->post('inbox_ids');   
		$inbox_action=$this->input->post('inbox_action');
...
function events() 
	 {
	   ...
	   if($this->form_validation->run('editevent') == FALSE) {
			...
			if($update){
				$subuserlist = $this->model->get_data('*','tbl_event_subscribe',  array('eve_id' => $data['eventId']));
                
                if(!empty($subuserlist)){
					foreach ($subuserlist as $s_u_list) {
						$eventnot['user_id'] = $s_u_list->eve_sub_user_id;
						$eventnot['n_type'] = 'event';
						$eventnot['n_cat_type'] = 'event';
		                $eventnot['n_type_id'] = $data['eventId'];
		                $eventnot['n_date'] = date("Y-m-d H:i:s");
		                $inserted = $this->model->insert_data('tbl_notification',$eventnot);
...
function faq(){
   ...
   function create_dir( $dir_name ){
	...
	function changeStatus(){
		...
		if($tablename == 'tbl_trail_type_master'){
      ...
      /****************** forum_category********************/
		...
		if($tablename == 'tbl_classified_list'){
			...
			if($statusChange == 'Approved'){
					...
					$message = str_replace("{page_link}",base_url().'classified/details/'.$getUserId->url_slag,$message);
                    $message = str_replace("{classified_name}",$getUserId->classified_created_by,$message);
		...
		if($tablename == 'tbl_vacation_list'){
			...
			if($status == 1){
				...
				if($statusChange == 'Approved'){
	                ...
	                $status= 0;
	                $userName = $getUserEmail->fname;
					$row1 =  $this->model->get_row('email_templates',array('title'=>'RentalCorrectionNeed'));
					$subject = $row1->subject;
					$message = $row1->content;
					$description = $vac_message;
					$pageLike = base_url().'lodging/'.$getUserId->vac_slag;
		...
		if($tablename == 'tbl_review'){
            ...
            $r_notifydata['n_cat_type']= 'rental';
            ...
            $b_notifydata['n_cat_type']= 'rental';
    ...
    function deletefun()
 	{    
		...
		/******************Delete tbl_google_ads********************/

 		if($tablename == 'tbl_google_ads'){
			$where=array('script_id'=> $id);
		}
		/******************Delete deletetrailreport********************/
		...
		/******************business List********************/
		if($tablename == 'tbl_trail_type_master'){
            $poiDetail = $this->model->get_row('tbl_trail_type_master', array('trail_id'=>$id));
			$Path_icon='./'.$poiDetail->trail_marker;
			unlink($Path_icon);
...
function updateVacData($id,$path) {

	      $id = $this->input->post('id');
		  $tablename = $this->input->post('tablename');

		if($tablename == 'tbl_update_vacation_list'){
            $statusChange = $this->input->post('statusChange');
			if($statusChange == 'Approved'){
                $getclonedata = $this->model->get_row('tbl_update_vacation_list', array('vac_id' =>$id));

				$businessadd['pl_id'] = $getclonedata->pl_id;
			    $businessadd['vac_name'] = $getclonedata->vac_name;
			    $businessadd['vac_slag'] = $getclonedata->vac_slag;
				$businessadd['vac_type'] = $getclonedata->vac_type;
				$businessadd['vac_contact'] = $getclonedata->vac_contact;
				$businessadd['vac_email'] = $getclonedata->vac_email;
				$businessadd['vac_address'] = $getclonedata->vac_address;
				$businessadd['vac_weekly_rate'] = $getclonedata->vac_weekly_rate;
				$businessadd['vac_zip_code'] = $getclonedata->vac_zip_code;
				$businessadd['vac_city'] = $getclonedata->vac_city;
				$businessadd['vac_wedsite_url'] = $getclonedata->vac_wedsite_url;
				$businessadd['vac_no_of_bedroom'] = $getclonedata->vac_no_of_bedroom;
				$businessadd['vac_bathroom'] = $getclonedata->vac_bathroom;
				$businessadd['vac_sleep'] = $getclonedata->vac_sleep;
				$businessadd['vac_price'] =  $getclonedata->vac_price;
				$businessadd['vac_location_type'] = $getclonedata->vac_location_type;
			    $businessadd['vac_floor_area'] = $getclonedata->vac_floor_area;
				$businessadd['vac_description'] = $getclonedata->vac_description;
                $businessadd['adv_filter_fields']=$getclonedata->adv_filter_fields;
				$businessadd['vac_lat'] = $getclonedata->vac_lat;
				$businessadd['vac_lang'] = $getclonedata->vac_lang;
				$businessadd['update_status'] = 0;
                ...
                if($update){
					...
					$message = str_replace("{rental_name}",ucfirst($getUserId->vac_name),$message);
					$message = str_replace("{page_link}",$pageLike,$message);
					$message = str_replace("{days_expire}",date_format(date_create($getUserId->vac_expiry_date), 'd M Y'),$message);
					$message = str_replace("{user_email_address}",$getUserEmail->email,$message);
					$this->sendingMail($getUserEmail->email ,$subject, $message);
                	$this->model->delete('tbl_update_vacation_list', array('vac_id' => $id));
                }
		    }else{
				$getUserId = $this->model->get_row('tbl_vacation_list', array('vac_id' => $id));
				$userID = $getUserId->user_id;
...
function seo_setting(){
	...
	/**
	 * POI Type Page for this controller.
	 */
	...
	function poi_type() 
	 {
		...
		$data['checkpers'] = $this->model->join_where('*', 'tbl_role_permission','tbl_user_assign_permission','role_id','role_id',array('tbl_user_assign_permission.user_id'=>$data['u_id']),'tbl_user_assign_permission.p_id','desc');
		$data['title'] = 'Admin | POI Type List';
		$data['active_tab_poi'] = 'active';
		$data['role'] = $this->data['siterole_id'];
		$data['basesegment']= $this->uri->segment(2);
		$data['segment'] = $this->uri->segment(3);
		//print_r($data['trailList']);
        $where = array(1 => 1);
        $data['kmlList'] = $this->model->get_data_order_by('*','tbl_trail_type_master', $where, 'trail_id','DESC');		
        if($data['segment'] == 'edit') {
       	if (isset($result)) {
		  	if($result[0]->permission_id == 3){
		  		if($result[0]->edit_permission !=1){
		...
		$data['poiID']= $this->uri->segment(4);
        $where = array('trail_id' => $data['poiID']);
        $data['poiDetail'] = $this->model->get_data('tbl_trail_type_master.*', 'tbl_trail_type_master',$where);
	  
	  	if($this->form_validation->run('editPoiType') == FALSE) {
		
		
			$this->load->view('administrator/include/inner_header',$data);
		    $this->load->view('administrator/POI/poi_type_add',$data);
		    $this->load->view('administrator/include/inner_footer');
	
		} else {
			
			$addpoi['trail_name'] = $this->input->post('poi_type');
			$Path_icon='./'.$data['poiDetail'][0]->trail_marker;
			unlink($Path_icon);
			if(!empty($_FILES['userfile']['name'])){
					

					$path_parts = pathinfo($_FILES["userfile"]["name"]);
					$image_path = $addpoi['trail_name'].'.'.$path_parts['extension'];
					...
					return redirect('administrator/poitypes/add');

					} else{

					
					$addpoi['trail_marker']='assets/images/map_icon/icon/'.$responce_image;
					}
				}

			$where= array('trail_id'=>$data['poiID']);

			$update = $this->model->update('tbl_trail_type_master',$addpoi,$where);
			//echo $this->db->last_query();
			if($update){
				$this->session->set_flashdata('success', 'The POI Type is Successfully update');
				redirect(base_url().'administrator/poitypes');
			}
		   ...
		   if($this->form_validation->run('editPoiType') == FALSE) {
				...
				$update = $this->model->insert_data('tbl_trail_type_master',$addpoi);
				//echo $this->db->last_query();
				if($update){
					$this->session->set_flashdata('success', 'The POI Type is Successfully Added');
					redirect(base_url().'administrator/poitypes');
				}
			...
			$this->load->view('administrator/include/inner_header',$data);
	        $this->load->view('administrator/POI/poi_type',$data);
	        $this->load->view('administrator/include/inner_footer');
...
$Path='./assets/images/map_icon/icon/';
...
function timezone(){
	...
	$data['userinfo']=$info;	
	if($data['u_id'] !=1){
	$result = role_permission($data['u_id'],34);
		if (empty($result)) {
			 redirect(base_url().'access_denied');
		}
    }	
	$data['active_tab_timezone'] = 'active';
	$data['basesegment']= $this->uri->segment(2);
	$data['checkpers'] = $this->model->join_where('*', 'tbl_role_permission','tbl_user_assign_permission','role_id','role_id',array('tbl_user_assign_permission.user_id'=>$data['u_id']),'tbl_user_assign_permission.p_id','desc');
	$data['role'] = $this->data['siterole_id'];
    $data['title'] = 'Time Zone';
    $data['zoneList'] = $this->model->get_data('*', 'zone',array(1 => 1));
	$data['timezone'] = $this->model->get_row('tbl_timezone',array('id' => 1));
	if($this->form_validation->run('timezone') == FALSE)
	{

		$this->load->view('administrator/include/inner_header',$data);
		$this->load->view('administrator/google_ads/timezone',$data);
		$this->load->view('administrator/include/inner_footer');

	} else {

		$timezone['country'] = $this->input->post('zone_name');
		$timezone['date'] = date('Y-m-d h:i:s');
		$where=array('id'=>1);
		$inserted = $this->model->update('tbl_timezone', $timezone,$where);

		if($inserted){

		  $this->session->set_flashdata('success', 'Updated succussfully.');
		}	
		redirect(base_url().'administrator/set-time-zone');	
	}
}
