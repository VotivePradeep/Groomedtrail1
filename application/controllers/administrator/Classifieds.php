...
public function classifiedslist(){
    ...
    if($data['segment'] == 'approve'){
        ...
        $query = $this->db->query('SELECT * FROM `tbl_classified_list` WHERE  `classified_status` = 1 and classified_expired >= CURDATE()  ORDER BY `classified_id` DESC');
         $data['classifiedList'] = $query->result();
		...
		$this->load->view('administrator/include/inner_footer');
    	 ...
    	 $query = $this->db->query('SELECT * FROM `tbl_classified_list` WHERE  `classified_status` = 0 and classified_expired < CURDATE() and classified_expired !="" ORDER BY `classified_id` DESC');
         $data['classifiedList'] = $query->result();
    	
    	$this->load->view('administrator/include/inner_header',$data);
		$this->load->view('administrator/classifieds/expired_classifiedslist',$data);
		$this->load->view('administrator/include/inner_footer');
