...
public function vacationlist(){
	...
	if($data['segment'] == 'pendingapprovals'){
     ...
     $query = $this->db->query("SELECT  `plan_master` . * ,  `tbl_vacation_list` . * ,  `tbl_user_master`.`lname` ,  `tbl_user_master`.`fname` FROM  `tbl_vacation_list` LEFT JOIN  `tbl_user_master` ON  `tbl_vacation_list`.`user_id` =  `tbl_user_master`.`user_id` LEFT JOIN  `plan_master` ON  `plan_master`.`pl_id` =  `tbl_vacation_list`.`pl_id` WHERE  `tbl_vacation_list`.`vac_status` =0 AND  `tbl_vacation_list`.`vac_payment_status` =1 AND  `tbl_vacation_list`.`user_id` !=1 AND  `plan_master`.`pl_price` !=0 ORDER BY  `vac_id` DESC ");
          $data['businessList'] = $query->result();
     ...
     $query = $this->db->query("SELECT  `plan_master` . * ,  `tbl_vacation_list` . * ,  `tbl_user_master`.`lname` ,  `tbl_user_master`.`fname` FROM  `tbl_vacation_list` LEFT JOIN  `tbl_user_master` ON  `tbl_vacation_list`.`user_id` =  `tbl_user_master`.`user_id` LEFT JOIN  `plan_master` ON  `plan_master`.`pl_id` =  `tbl_vacation_list`.`pl_id` WHERE  `tbl_vacation_list`.`vac_status` =0  AND  `tbl_vacation_list`.`user_id` !=1 AND  `plan_master`.`pl_price` =0 ORDER BY  `vac_id` DESC ");
	  ...
	  $query = $this->db->query("SELECT  `plan_master` . * ,  `tbl_vacation_list` . * ,  `tbl_user_master`.`lname` ,  `tbl_user_master`.`fname` FROM  `tbl_vacation_list` LEFT JOIN  `tbl_user_master` ON  `tbl_vacation_list`.`user_id` =  `tbl_user_master`.`user_id` LEFT JOIN  `plan_master` ON  `plan_master`.`pl_id` =  `tbl_vacation_list`.`pl_id` WHERE  `tbl_vacation_list`.`vac_status` =0 AND  `tbl_vacation_list`.`vac_payment_status` =0 AND  `tbl_vacation_list`.`user_id` !=1 AND  `tbl_vacation_list`.`vac_expiry_date` >= CURDATE() AND  `plan_master`.`pl_price` !=0 ORDER BY  `vac_id` DESC ");
          $data['businessList'] = $query->result();
		////////////////////////total user////////
		$this->load->view('administrator/include/inner_header',$data);
...
public function addvacationpage(){
		...
		if($this->form_validation->run('addvacbusin') == FALSE) {
			...
			$this->load->view('administrator/include/inner_footer');
				...
				/*  if(isset($_FILES)){
					$vacImage = $this->insertMultipleImage('vac_imag', 'upload/business_image/vacational/');
					 if(isset($vacImage) && !empty($vacImage))
					 {   
					   foreach($vacImage as $imagesG)
						{
